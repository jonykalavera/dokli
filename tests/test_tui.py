...
def test_browser_yank_copies_selected_id(mocker):
    """We expect 'y' to copy the selected record's primary id."""
    _patch_api(mocker)
    registry = parse_spec(FAKE_SCHEMA)
...
def test_browser_yank_notifies_copied_id(mocker):
    ...
    async def main():
        app = DokliApp(config=_config())
        async with app.run_test() as pilot:
            app.copy_to_clipboard = lambda value: None
            await _mount_browser(app, pilot, _connection(), registry)
            screen = app.screen
            notify = mocker.spy(screen, "notify")
            _select(app, "project")
            await pilot.pause()
            await pilot.press("enter")
            await _wait_for_label(app, pilot, "media")
            ...
            await pilot.press("y")
            await pilot.pause()
            assert any("Copied p1" in str(call) for call in notify.call_args_list)

    _run(main())


def test_browser_yank_no_id_warns(mocker):
    """We expect yank on a record without an id to warn."""
    _patch_api(mocker)
    registry = parse_spec(FAKE_SCHEMA)
    ...
    async def main():
        ...
        async with app.run_test() as pilot:
            ...
            screen = app.screen
            notify = mocker.spy(screen, "notify")
            screen.path.append(Level(kind="children", items=[{"_kind": "thing", "name": "noid"}], entity="thing"))
            await pilot.pause()
            await pilot.press("y")
            await pilot.pause()
            assert any("No id available" in str(call) for call in notify.call_args_list)
