...
- `dokli ls [<conn>] [--type TYPE] [--search TERM] [--format agent]` — flat list of every service and its id (project/environment/type/name/appName), handy for scripting and agents; `--type` filters by service type, `--search` by name.
- **`--format agent`** (NDJSON dataframe) is available on `api`, `stats`, `ls` and `state`: line 1 is the column names, every following line is one record's values (arrays). Nested objects flatten with `__`. `json.load`-parseable per line — made for agents and scripting.
